...
class TestLocations(unittest.TestCase):
    ...
    def setUp(self):
        self.location_ids = []
        self.location_ids.append(api.location_create(
            "unittest_location", contact="testing")["id"])
    ...
    def test_location_delete(self):
        self.assertEqual(api.location_delete(
            self.location_ids[0]), self.location_ids[0])
        self.location_ids.pop()

    def test_location_resolve_to_id(self):
        self.assertEqual(api.location_resolve_to_id(
            "unittest_location"), self.location_ids[0])

    def test_location_unique_id(self):
        self.logger.setLevel(logging.CRITICAL)
        with self.assertRaises(SEPSesamAPIError):
            self.assertEqual(
                api.location_create("second_unittest_location",
                                    **{"id": self.location_ids[0]})["error"],
                "duplicate.entry",
            )

    def test_location_get(self):
        self.assertEqual(api.location_get("unittest_location")
                         ["id"], self.location_ids[0])
    ...
    def test_location_find(self):
        id_now = api.location_create(
            "third_unittest_location", parentId=self.location_ids[0])["id"]
        self.location_ids.append(id_now)
    ...
    def test_location_update(self):
        self.assertEqual(api.location_get(
            "unittest_location")["contact"], "testing")
        api.location_update(
            self.location_ids[0], "unittest_location", contact="testing_done")
        self.assertEqual(api.location_get("unittest_location")
                         ["contact"], "testing_done")
