...
class TestBackupEvents(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.drive_groups = []
        self.create_backup_event_initial("unittest_backup_event", "unittest_backup_task_be", "unittest_schedule_be", "unittest_dg_be", "unittest_mp_be")
    ...
    def test_backup_event_delete(self):
        self.assertEqual(api.backup_event_delete(self.backup_event_ids[0]), int(self.backup_event_ids[0]))
    ...
    def test_backup_event_get(self):
        self.assertEqual(api.backup_event_get(self.backup_event_ids[0])["name"], "unittest_backup_event")

    def test_backup_event_find(self):
        self.assertTrue(len(api.backup_event_find(**{"taskName":"unittest_backup_task_be"})) == 1) 

        self._create_backup_event_solo("second_unittest_backup_event")
        self.assertTrue(len(api.backup_event_find(**{"taskName":"unittest_backup_task_be"})) == 2) 
        ...
        self.create_backup_event_solo("third_unittest_backup_event", "second_unittest_backup_task_be", self.schedules[0], self.media_pools[0])
        self.assertTrue(len(api.backup_event_find(**{"taskName":"unittest_backup_task_be"})) == 2) 
    ...
    def create_backup_event_initial(self, backup_event_name, backup_task_name, schedule_name, drive_group_name, media_pool_name):
        ...
        self.drive_groups.append(int(api.drive_group_create( drive_group_name)["id"]))
        ...
        self.create_backup_event_solo(backup_event_name, backup_task_name, schedule_name, media_pool_name)
        
    def _create_backup_event_solo(self, backup_event_name):
        self.create_backup_event_solo(backup_event_name, self.backup_tasks[0], self.schedules[0], self.media_pools[0])
