...
class TestMedia(unittest.TestCase):
    ...
    def test_media_find(self):
        self.assertEqual(len(api.media_find(**{"pool": [str(self.media_pools[0])]})), 1)
        self.create_media("unittest_media_2", self.media_pools[0])
        self.assertEqual(len(api.media_find(**{"pool": [str(self.media_pools[0])]})), 2)
        self.assertTrue(
            any(
    ...
    def test_media_update(self):
        self.assertEqual(
            api.media_update(self.media_ids[0], "UNLOCKED", "LTO_Tape", self.media[0] , self.media_pools[0], 1503698400000, **{"label":"New_Label"})["label"], 
            "New_Label"
            )
        self.create_media_pool("unittest_mp_2", self.drive_groups[0])
        self.assertEqual(
            api.media_update(self.media_ids[0], "UNLOCKED", "LTO_Tape", self.media[0] , self.media_pools[1], 1503698400000, )["poolName"], 
            "unittest_mp_2"
            )
