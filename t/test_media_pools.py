...
class TestMediaPools(unittest.TestCase):
    ...
    def test_media_pool_delete(self):
        self.assertEqual(api.media_pool_delete(self.media_pools[0]), self.media_pools[0])
        self.media_pools.pop(0)

    def test_media_pool_find(self):
        self.assertEqual(len(api.media_pool_find(**{"driveGroups": [str(self.drive_groups[0])]})), 1)
        self.createMediaPool("unittest_mp_2", self.drive_groups[0])
        self.assertEqual(len(api.media_pool_find(**{"driveGroups": [str(self.drive_groups[0])]})), 2)
        self.assertTrue(
            any(
    ...
    def test_media_pool_get(self):
        self.assertEqual(
            api.media_pool_get(self.media_pools[0])["descript"], "unittest_media_pool_description"
        )
    ...
    def test_media_pool_update(self):
        ...
        self.assertEqual(
            api.media_pool_update(self.media_pool_ids[0], self.media_pools[0], self.drive_groups[0], **{"descript":"new_unittest_description"})["descript"], 
            "new_unittest_description"
            )
        self.assertEqual(
            api.media_pool_update(self.media_pool_ids[0], self.media_pools[0], new_dg)["driveGroupId"], 
            new_dg
            )
            
    #helper methods
    def createMediaPool(self, name, id):
        result = api.media_pool_create(name, **{"driveGroupId": id, "descript":"unittest_media_pool_description"})
